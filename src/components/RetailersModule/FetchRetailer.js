...
class FetchRetailer extends React.Component {
    ...
    viewCrop(RetstatusId) {
        let ViewPage = <select >
            ...
            <option value="0" selected={RetstatusId}>{window.strings.RETAILERS.PENDING}</option>
            <option value="1">{window.strings.RETAILERS.ACCEPTED}</option>
            <option value="2">{window.strings.RETAILERS.REJECTED}</option>
        </select>
        return ViewPage;
    }
